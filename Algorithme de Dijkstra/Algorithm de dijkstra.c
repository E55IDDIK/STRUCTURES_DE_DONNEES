...
int main() {
    ...
    for (i = 1; i <= m; ++i) {
        printf("saisie ligne et colon de l'element de nombre %d : \n ",i);
        scanf("%d %d", &u, &v);
        printf("saisi valeur de l'element : \n");
        scanf("%d",&w);
        adj[u][v] = w;
        adj[v][u] = w;
    }
    ...
    dijkstra(start);
     min = distance[1];
    for (i = 1; i <= n; ++i) {
        printf("Distance entre %d et %d is %d\n", start, i, distance[i]);
        if(distance[i]<min){
            min =  distance[i] ;
            a = i ;
        }
    }
    printf("Ainsi, le chemin le plus court est le chemin depuis %d to %d est %d",start,a,min);
    return 0;
}
